...
function discoverSemanasSheets(sheetTitles) {
  const found = sheetTitles
    .map(title => String(title || '').trim())
    .filter(title => /^SEMANAS\s+(AI|MO)(?:\s+.*)?$/i.test(title))
    .map(title => {
      const empresa = title.match(/^SEMANAS\s+(AI|MO)/i)?.[1]?.toUpperCase()
      return { empresa, title, range: `${title}!A1:M500` }
    })
    .filter(s => s.empresa)

  const hasAi = found.some(s => s.empresa === 'AI')
  const hasMo = found.some(s => s.empresa === 'MO')

  if (!hasAi) found.push({ empresa: 'AI', title: 'SEMANAS AI', range: RANGES.SEMANAS_AI })
  if (!hasMo) found.push({ empresa: 'MO', title: 'SEMANAS MO', range: RANGES.SEMANAS_MO })
...
export async function getAllDashData() {
  ...
  const semanasSheets = discoverSemanasSheets(sheetTitles)

  const [rawForecastAi, rawForecastMo, rawReunioesGeral, ...restRaw] = await Promise.all([
    fetchRange(RANGES.FORECAST_AI),
    fetchRange(RANGES.FORECAST_MO),
    fetchRange(RANGES.REUNIOES_GERAL),
    ...semanasSheets.map(s => fetchRange(s.range)),
    ...dashSheets.map(s => fetchRange(s.range)),
  ])

  const semanasRaw = restRaw.slice(0, semanasSheets.length)
  const dashRaw = restRaw.slice(semanasSheets.length)
  ...
  const semanasAi = parseSemanasSet('AI')
  const semanasMo = parseSemanasSet('MO')
...
export async function getDebugForecastData() {
  ...
  const { sheets: dashSheets, periodos } = discoverDashPeriods(sheetTitles)
  const semanasSheets = discoverSemanasSheets(sheetTitles)
  const [rawForecastAi, rawForecastMo, rawReunioesGeral, ...rawSemanas] = await Promise.all([
    fetchRange(RANGES.FORECAST_AI),
    fetchRange(RANGES.FORECAST_MO),
    fetchRange(RANGES.REUNIOES_GERAL),
    ...semanasSheets.map(s => fetchRange(s.range)),
  ...
  return {
    ...
    raw: {
      ...
      SEMANAS_SAMPLE: rawSemanas.map((rows, idx) => ({ sheet: semanasSheets[idx], rows: rows.slice(0, 8) })),
    },
    parsed: {
