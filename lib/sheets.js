...
const RANGES = {
  ...
  FORECAST_AI: 'FORECAST_AI!A1:N20',
  FORECAST_MO: 'FORECAST_MO!A1:N20',
}
...
function parseNum(val) {
  ...
  // Handles formats like (R$ 1.234,56) and unicode minus.
  if (/^\(.*\)$/.test(s)) {
    negative = true
    s = s.replace(/[()]/g, '')
  }
  ...
  // Remove currency, percentage, spaces and any text that is not numeric punctuation.
  s = s
    .replace(/R\$\s*/gi, '')
    .replace(/%/g, '')
    .replace(/\s/g, '')
  ...
  // Handles both BR and US formats by treating the rightmost separator as decimal.
  // BR: 46.664,7 -> 46664.7 | US/API: 46,664.7 -> 46664.7
  if (lastComma !== -1 && lastDot !== -1) {
    ...
    if (parts.length > 2) {
      // 1.234.567 or 1,234,567 -> thousands only
      s = parts.join('')
    } else if (decimals.length === 3 && parts[0].length <= 3) {
      // 2.500 / 2,500 -> likely thousands, not decimal
      s = parts.join('')
    } else {
      // 46,6 / 46.6 / 46664,7 / 46664.7 -> decimal
      s = parts[0] + '.' + decimals
    }
  }
...
function parseForecast(rows) {
  // Cols by position: A=MES, B=MRR PAGO PROJ, C=% PAGO PROJ, D=GAP PAGO PROJ,
  // E=PROJECAO VENDIDO, F=% VENDIDO PROJ, G=GAP NMRR, H=GAP CONTRATOS,
  // I=GAP RLZD, J=GAP AGD, K=META DIA PAGO, L=META AGD DIA, M=META RLZD DIA, N=META CONT PAGO DIA
  ...
  return rows
    ...
    .filter(r => {
      ...
      return mes !== 'MÊS' && mes !== 'MES'
    })
    .map(r => ({
      mes: String(r[0] || '').trim(),
      mrrPago: parseNum(r[1]),
      pctPago: parseNum(r[2]),
      gapPago: String(r[3] || ''),
      projecaoVendido: parseNum(r[4]),
      pctVendido: parseNum(r[5]),
      gapNmrr: String(r[6] || ''),
      gapContratos: parseNum(r[7]),
      gapRlzd: parseNum(r[8]),
      gapAgd: parseNum(r[9]),
      metaDiaPago: parseNum(r[10]),
      metaAgdDia: parseNum(r[11]),
      metaRlzdDia: parseNum(r[12]),
      metaContPagoDia: parseNum(r[13]),
    }))
}
