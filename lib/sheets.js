...
function parseNum(val) {
  if (!val || val === '-' || val === '#DIV/0!' || val === '') return 0
  let s = String(val).trim()
  // Remove R$, spaces, %
  s = s.replace(/R\$\s*/g, '').replace(/%/g, '').trim()
  // Brazilian format: 1.234,56 → 1234.56
  if (s.includes(',') && s.includes('.')) {
    // has both: dot = thousands, comma = decimal
...
function processReunioes(rows, servicoCol = 4) {
  ...
  data.forEach(r => {
    ...
    const v = parseFloat(String(r[8] || '0').replace(/[^0-9.,]/g, '').replace(',', '.')) || 0
...
function parseForecast(rows) {
  ...
  return rows.slice(1).filter(r => r && r[0] && String(r[0]).trim() !== '').map(r => ({
    mes: String(r[0] || '').trim(),
