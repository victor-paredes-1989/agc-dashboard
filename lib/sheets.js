...
function parseForecast(rows) {
  ...
  return rows.slice(1).filter(r => r[0] && r[0].trim()).map(r => {
    ...
    return {
      mes: obj['MÊS'] || obj['MES'] || r[0] || '',
      mrrPago: parseNum(obj['MRR PAGO PROJETADO']),
      pctPago: parseNum(obj['% PAGO PROJETADO']),
      gapPago: obj['GAP PAGO PROJETADO'] || '',
      projecaoVendido: parseNum(obj['PROJEÇÃO VENDIDO'] || obj['PROJECAO VENDIDO']),
      pctVendido: parseNum(obj['% VENDIDO PROJETADO']),
      gapNmrr: obj['GAP NMRR'] || '',
      gapContratos: parseNum(obj['GAP CONTRATOS']),
      gapRlzd: parseNum(obj['GAP RLZD']),
      gapAgd: parseNum(obj['GAP AGD']),
      metaDiaPago: parseNum(obj['META DIA PAGO']),
      metaAgdDia: parseNum(obj['META AGD DIA']),
      metaRlzdDia: parseNum(obj['META RLZD DIA']),
      metaContPagoDia: parseNum(obj['META CONT. PAGO DIA']),
    }
  })
}
