...
async function getAccessToken() {
  ...
  // .sign() retorna Buffer; convertemos para base64url manualmente
...
export async function getAllDashData({ force = false } = {}) {
  ...
  try {
    ...
    // Nunca retornar 500 — logar o erro e retornar estrutura vazia compatível
...
export async function getDebugInfo() {
  const envCheck = {
    GOOGLE_SHEET_ID: SHEET_ID ? `ok (${SHEET_ID.slice(0,10)}...)` : 'FALTANDO',
    GOOGLE_SERVICE_ACCOUNT_EMAIL: SERVICE_ACCOUNT_EMAIL ? `ok (${SERVICE_ACCOUNT_EMAIL})` : 'FALTANDO',
    GOOGLE_PRIVATE_KEY: PRIVATE_KEY_RAW
      ? `ok (len=${String(PRIVATE_KEY_RAW).length}, has_header=${String(PRIVATE_KEY_RAW).includes('PRIVATE KEY')})`
      : 'FALTANDO',
  }
  ...
  try {
    const token = await getAccessToken()
    authTest = `ok (token len=${token.length})`
  } catch (e) {
    authTest = `ERRO: ${e.message}`
  }
  ...
  return { envCheck, authTest, sheetTitles: titlesTest }
}
