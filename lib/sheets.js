...
export async function getAllDashData({ force = false } = {}) {
  ...
  try {
    ...
    const [rawReunioesGeral, rawPerformanceOrigem, ...restRaw] = await Promise.all([
      ...
      // TCV_MENSAL / SDR_FAT — abas novas, já criadas manualmente na MASTER. Safe-fail:
      // se estiverem vazias/ausentes, os parsers abaixo devolvem [] e nada quebra.
    ...
    const forecastRaw = restRaw.slice(3, 3 + forecastRanges.length)
    const semanasRaw = restRaw.slice(3 + forecastRanges.length, 3 + forecastRanges.length + semanasSheets.length)
    const dashRaw = restRaw.slice(3 + forecastRanges.length + semanasSheets.length)
