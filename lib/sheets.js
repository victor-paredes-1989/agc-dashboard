...
// Mapa de nome completo do mês (normalizado sem acentos) → número 1-12.
// Usado para validar a coluna DATA contra MES/ANO em parseReunioesGeral.
const MONTH_NAME_TO_NUM = Object.fromEntries(
  Object.values(MONTHS).map(m => [
    m.nome.normalize('NFD').replace(/[̀-ͯ]/g, ''),
    m.numero,
  ])
)

// Extrai {month, year} da coluna DATA do REUNIOES_GERAL.
// Suporta: DD/MM, DD/MM/AA, DD/MM/AAAA (padrão BR — NUNCA inverte dia/mês),
//          serial numérico do Google Sheets (dias desde 30/12/1899, UTC).
// Retorna null para valor vazio ou formato não reconhecido.
// Se apenas DD/MM for fornecido (sem ano), year = null (ano não verificável).
function extractDataMonthYear(value) {
  if (value === null || value === undefined || value === '') return null
  // Serial numérico do Sheets (número > 1 000 → é uma data, não um dia do mês)
  const num = Number(value)
  if (!isNaN(num) && String(value).trim() !== '' && num > 1000) {
    // Converter serial para data UTC: época base = 30/12/1899
  ...
  if (!s) return null
  // Formato BR: DD/MM ou DD/MM/AA ou DD/MM/AAAA
  ...
  if (isoMatch) {
    ...
    if (month < 1 || month > 12) return null
    return { month, year: Number(isoMatch[1]) }
  }
  return null
}
...
function parseReunioesGeral(rows) {
  ...
  return rows.slice(1)
    .filter(r => {
      ...
      // ── Regra de integridade de data ────────────────────────────
      // Uma reunião só entra no mês da aba se MÊS(DATA) = mês da aba
      // e ANO(DATA) = ano da aba.  DATA vazia ou não reconhecível: mantém
      // o comportamento anterior (inclui), pois não é possível validar.
      const mesNomeRaw = String(r[COL.mes] || '').trim().toUpperCase()
        .normalize('NFD').replace(/[̀-ͯ]/g, '')
      const anoRaw = String(r[COL.ano] || '').trim()
      const mesNumAba = MONTH_NAME_TO_NUM[mesNomeRaw]
      if (mesNumAba) {
        const dataRaw = r[COL.data]
        const parsed = extractDataMonthYear(dataRaw)
        if (parsed) {
          // Mês não coincide → excluir
          if (parsed.month !== mesNumAba) return false
          // Ano disponível na DATA e não coincide → excluir
          if (parsed.year !== null && anoRaw && parsed.year !== Number(anoRaw)) return false
        }
        // parsed === null (DATA vazia/inválida): manter linha sem filtrar
      }
