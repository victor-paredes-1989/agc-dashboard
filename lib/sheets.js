...
export async function getAllDashData() {
  ...
  const [rawReunioesGeral, ...restRaw] = await Promise.all([
    fetchRange(`${config.reunioesGeral}!A1:N20000`),
  ...
  const forecastRaw = restRaw.slice(0, forecastRanges.length)
  const semanasRaw = restRaw.slice(forecastRanges.length, forecastRanges.length + semanasSheets.length)
  const dashRaw = restRaw.slice(forecastRanges.length + semanasSheets.length)
...
export async function getDebugForecastData() {
  ...
  const [rawReunioesGeral, ...rest] = await Promise.all([
    fetchRangeSafe(`${config.reunioesGeral}!A1:N20000`),
    ...forecastRanges.map(f => fetchRangeSafe(f.range)),
    ...semanasSheets.map(s => fetchRangeSafe(s.range)),
  ])
