...
// requireSeniorOrAdmin — guard para áreas restritas de VISUALIZAÇÃO (ex.: Investimento em
// Equipe), nunca para funcionalidades administrativas. SENIOR enxerga essas áreas mas não
// deve, em nenhuma hipótese, passar por requireAdmin — as duas checagens são deliberadamente
// independentes (SENIOR nunca "vira" ADMIN aqui, e um endpoint administrativo continua
// exigindo requireAdmin explicitamente, nunca este helper).
export async function requireSeniorOrAdmin(req, res) {
  const user = await requireActiveUser(req, res)
  if (!user) return null // resposta já enviada por requireActiveUser

  if (user.perfil !== 'ADMIN' && user.perfil !== 'SENIOR') {
    res.status(403).json({ error: 'senior_or_admin_required' })
    return null
  }
  return user
}
