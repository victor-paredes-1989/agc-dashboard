...
//
// perfil ∈ {'ADMIN','USUARIO'} — qualquer outro valor é rejeitado antes de tocar a
// planilha. actorEmail é OBRIGATÓRIO pelo mesmo motivo de updateUserStatus.
//
// Guardas, nesta ordem:
...
//   5. rebaixar ADMIN ATIVO que é o único restante    -> last_admin_protected
export async function updateUserPerfil({ email, perfil, actorEmail }) {
  ...
  if (found.record.perfil === 'ADMIN' && nextPerfil === 'USUARIO' && found.record.status === 'ATIVO') {
    const wouldOrphan = await wouldRemoveLastActiveAdmin(target)
    if (wouldOrphan) return { updated: false, reason: 'last_admin_protected' }
  }
